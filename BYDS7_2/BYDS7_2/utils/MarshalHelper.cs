using BYDS7_2.utils.atttribute;
...
namespace BYDS7_2.utils
{
    public static class MarshalHelper
    {
        ...
        public static T BytesToStruct<T>(byte[] rawData)  //目前只用到这个方法
        {
            T val = default(T);
            ...
            try
            {
                return (T)Marshal.PtrToStructure(gCHandle.AddrOfPinnedObject(), typeof(T));//进行直接内存映射
            }
            finally
            {
                gCHandle.Free();
